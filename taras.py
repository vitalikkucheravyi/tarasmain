...
TOKEN = 'OTY5OTg5MzM1OTE3MTM3OTMw.GciDgO.irf2kdMAK2XR0OL-I1D62Wmgn_dI6pKZGqvNNw'

bot = commands.Bot(command_prefix=".")
bot.remove_command('help')
reddit = asyncpraw.Reddit(client_id = 'hqKoSwLZqYh2FH2vwHPb4w',
                        client_secret = '3TcNkhWdrsFpN26Q9zBS_4jqbrgx5w',
...
invite = "https://discord.com/api/oauth2/authorize?client_id=969989335917137930&permissions=8&scope=bot"
server_invite = "https://discord.gg/UuW4KmKQKu"
bot_creator = "Respected Guse#8625"
site = "https://sites.google.com/view/tarasbot/%D0%B3%D0%BE%D0%BB%D0%BE%D0%B2%D0%BD%D0%B0"

mems = []
timeout = 900 #1800
id_channel = 987779097675587674 #games
subreddit_name = 'memes'
POST_LIMIT = 1

@bot.event
async def on_ready():
    print('Бот працює! Бот: Тарас.')
    chanel = bot.get_channel(id_channel)
    while True:
        await bot.change_presence(status=discord.Status.idle, activity=discord.Game(f".допомога | {len(bot.guilds)} серверів"))
        await sleep(15) 
        ...
        if item.title not in mems:
            ...
            await chanel.send(item.url) 
            
@bot.event
async def on_guild_join(guild):
    emb = discord.Embed(title = "ТарасБот", colour = discord.Color.green())
    ...
    emb.add_field(name = ":face_with_monocle: Мої соц.мережі!", value = f"Технічний discord-сервер: {server_invite}\nМій сайт: {site}" )
...
async def on_message(message):
    ...
    if 'слава '+'україні' in message.content.lower():
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "На тобі лису", colour = 0xff9900)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Я не люблю себе показувати, но тобі можна!", colour = 0xff9900)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "На тобі кіт", colour = 0xff9900)
            emb.set_image(url = json_data['link'])
            await message.channel.send(embed = emb)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Песик", colour = 0xff9900)
            emb.set_image(url = json_data['link'])
            await message.channel.send(embed = emb)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Увага анекдот!", colour = discord.Color.green())
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Орки бояться азовців капець як! Слава нашим!", colour = 0xff9900)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Анонімус", colour = 0xff9900)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Наша оборона найкраща!", colour = discord.Color.green())
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Українські меми", colour = 0xff9900)
        ...
        if message.author.id == bot.user.id:
            ...
            emb = discord.Embed(title = "Таблиця множення", colour = discord.Color.green())
...
async def help(ctx):
    emb = discord.Embed(title = "Допомога | Інформація про боті", colour = discord.Color.green(), timestamp=ctx.message.created_at)
    emb.add_field(name="Команди бота:", value=f"{prefix}допомога(help)\n{prefix}команди(command)\n{prefix}інфо(get)\n{prefix}поцілунок(kiss)\n{prefix}питання(ball)\n{prefix}монетка(dice)\n{prefix}попіт(popit)\n{prefix}обійми(hug)\n{prefix}кохання(love)\n{prefix}слово(word)")
    emb.add_field(name="Префікс бота:", value=f"{prefix}")
    emb.add_field(name="Затримка бота", value=f"```{int(bot.ws.latency * 1000)}ms```")
    emb.add_field(name="Сервера", value=f"```{len(bot.guilds)}```")
    emb.add_field(name="Користувачі", value=f"```{len(bot.users)}```")
    emb.add_field(name="Розробник бота:", value=f"```{bot_creator}```")
    emb.add_field(name="Посилання на бота:", value=f"{invite}")
    emb.add_field(name="Посилання на технічний сервер:", value=f"{server_invite}")
    emb.add_field(name="Сайт бота:", value=f"{site}")
    emb.set_author(name=ctx.command.description)
    emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
    await ctx.reply(embed = emb)

@bot.command(name = "command", aliases = ["команди"])
async def command(ctx, arg = None):
    if arg is None:
        emb = discord.Embed(title = "Помилка!", colour = discord.Color.red(), timestamp=ctx.message.created_at)
        emb.add_field(name="Введіть команду!", value="Приклад: .command kiss(інформація о команді kiss)")
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)
    else:
        if arg == 'help' or arg == 'допомога':
            emb = discord.Embed(title = "Інформація о команді help", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}help(допомога) - команда допомого и вся інформація о боті.", value="Команди бота\nПрефікс бота\nЗатримка бота\n Кількість\
             сервері\nКількість користувачів\nРозробник бота\nПосилання на бота і тех.сервер\nЗапропонувати ідеї")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'get' or arg == 'інфо':
            emb = discord.Embed(title = "Інформація о команді get", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}get(інфо) - повна інформація о користувачі", value="ID користувача\nГолосовий канал\nДата регестрації")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'kiss' or arg == 'поцілунок':
            emb = discord.Embed(title = "Інформація о команді kiss", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}kiss(поцілунок) - команда для поцілунку користувача", value=f"Приклад: {prefix}kiss @Member")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'ball' or arg == 'питання':
            emb = discord.Embed(title = "Інформація о команді ball", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}ball(питання) - рандомна відповідь(Так/Ні) на питання", value=f"Приклад: {prefix}ball Питання")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'dice' or arg == 'монетка':
            emb = discord.Embed(title = "Інформація о команді dice", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}dice(монетка) - гра Орел і Решка. *УВАГА! Орел/Решка вводити з великої літери*", value=f"Приклад: {prefix}dice Орел/Решка")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'hug' or arg == 'обійми':
            emb = discord.Embed(title = "Інформація о команді hug(обійми)", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}dice - гра Орел і Решка. *УВАГА! Орел/Решка вводити з великої літери*", value=f"Приклад: {prefix}dice Орел/Решка")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)

            await ctx.reply(embed = emb)
        elif arg == 'popit' or arg == 'попіт':
            emb = discord.Embed(title = "Інформація о команді popit(попіт)", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}popit(попіт) - гра в Поп Іт", value=f"Приклад: {prefix}popit")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'love' or arg == 'кохання':
            emb = discord.Embed(title = "Інформація о команді love(кохання)", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}love(кохання) - відправити цікаву аніміцію)", value=f"Приклад: {prefix}love")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        elif arg == 'word' or arg == 'слово':
            emb = discord.Embed(title = "Інформація о команді word(слово)", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name=f"{prefix}word(слово) - відправити пропозицію о нового слова!", value=f"Приклад: {prefix}word")
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)

@bot.command(name = "get", aliases = ["інфо"])
async def get(ctx, member: discord.Member = None):
    member = member or ctx.author
    emb = discord.Embed(title = f"{member.display_name}", colour = discord.Color.green(), timestamp=ctx.message.created_at)
    emb.add_field(name="ID:", value=member.id)
    emb.add_field(name="Голосовий канал", value=f"{member.voice.channel.name} ({member.voice.channel.mention})"
                                                  if member.voice is not None else "Немає")
    emb.add_field(name="Зарегеструвався", value=member.created_at.strftime('```%d.%m.%Y %H:%M:%S```'))
...
@bot.command()
async def pm(ctx,member:discord.Member = None, *, arg = None):
    if ctx.author.id == 503904015638200340 or ctx.author.id == 660069567959400449:
        if member is None:
            emb = discord.Embed(title = 'Помилка', colour = discord.Color.red())
            emb.add_field(name = 'Введіть користувача!', value = f'Приклад: ***{prefix}pm Користувач Текст***')
            await ctx.reply(embed = emb)
            return
        else:
            if arg is None:
                emb = discord.Embed(title = 'Помилка', colour = discord.Color.red())
                emb.add_field(name = 'Введіть повідомлення', value = f'Приклад: ***{prefix}pm Користувач Текст***')
                await ctx.reply(embed = emb)
                return
            else:
                emb = discord.Embed(title = 'Повідомлення було відправлено!', colour = discord.Color.green(), timestamp=ctx.message.created_at)
                emb.add_field(name = 'Повідомлення:', value = f'{arg}')
                emb.set_author(name=ctx.command.description)
                emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
                await ctx.reply(embed = emb)
                emb = discord.Embed(title = 'Технічна підтримка бота : Тарас.', colour = discord.Color.green(), timestamp=ctx.message.created_at)
                emb.add_field(name = f'{arg}', value = f'Дякуємо вам за допомогу :)!')
...
async def say(ctx, *, arg):
    if ctx.author.id == 503904015638200340 or ctx.author.id == 660069567959400449:
        ...
        await ctx.reply("Ви не маєте право використовувати цю команду!")
...
async def send(ctx, *, message=None):
    if ctx.author.id == 503904015638200340 or ctx.author.id == 660069567959400449:
        ...
        await ctx.send("Ви не маєте доступ до кмд!")

@bot.command(name = "kiss", aliases = ["поцілунок"])
async def kiss(ctx, member: discord.Member = None):
    if member is None:
        emb = discord.Embed(title = "Помилка :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
        emb.add_field(name="Вкажіть кого поцілувати!", value=(f'{prefix}kiss <member>'))
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)
    else:
        if member == ctx.message.author:
            emb = discord.Embed(title = "Помилка :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name="Ти дебіл самого себе цілувати?!", value=(f'{prefix}kiss <member>'))
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        else: 
            ...
            emb = discord.Embed(title = f"Користучав `{ctx.author.display_name}` поцілував користувача `{member.display_name}`", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.set_image(url = f'{random.choice(possible_responses)}')
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Вітаю вас! ", icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)

@bot.command(name = "hug", aliases = ["обійми"])
async def hug(ctx, member: discord.Member = None):
    if member is None:
        emb = discord.Embed(title = "Помилка :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
        emb.add_field(name="Вкажіть кого поцілувати!", value=(f'{prefix}hug <member>'))
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)
    else:
        if member == ctx.message.author:
            emb = discord.Embed(title = "Помилка :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.add_field(name="Ти дебіл самого себе цілувати?!", value=(f'{prefix}hug <member>'))
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)
        else: 
            ...
            emb = discord.Embed(title = f"Користучав `{ctx.author.display_name}` обійняв користувача `{member.display_name}`", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            emb.set_image(url = f'{random.choice(possible_responses)}')
            emb.set_author(name=ctx.command.description)
            emb.set_footer(text="Вітаю вас! ", icon_url=ctx.author.avatar_url)
            await ctx.reply(embed = emb)

@bot.command(name = "word", aliases = ["слово"])
async def word(ctx, *, arg = None):
    CHANNEL_ID = 986336727755006002
    if arg is None:
        emb = discord.Embed(title = "Помилка :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
        emb.add_field(name="Введіть слово(і бажано відповідь на слово)!", value=(f'{prefix}word(слово) <нове слово> <яку відповідь ви б хотіли на це слово>'))
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)
    else:
        ...
        emb = discord.Embed(title = f"Запропоноване нове слово!", colour = discord.Color.green(), timestamp=ctx.message.created_at)
        emb.add_field(name=f"{arg}", value=(f'Запропонував: {ctx.author.display_name}'))
        emb.add_field(name="Інформація о користуваче:", value=(f'Ім`я користувача: {ctx.author.name}\nID користувача: {ctx.author.id}'))
        emb.add_field(name="Інформація о сервері:", value=(f'Назва сервера: {ctx.guild.name}\nID сервера: {ctx.guild.id}\nПосилання на сервер: https://discord.gg/{invite.code}'))
        emb.set_thumbnail(url = f"{ctx.guild.icon_url}")
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Запропнував: " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await channelid.send(embed = emb)
        await ctx.reply(f"Слово було відправлено!\nВаше слово: `{arg}`")

@bot.command(name = "popit", aliases = ["попіт"])
async def popit(ctx):
...
@bot.command(name = "ball", aliases = ["питання"])
async def ball(ctx, *, arg = None):
    if arg is None:
        emb = discord.Embed(title = 'Помилка', colour = discord.Color.red(), timestamp=ctx.message.created_at)
        emb.add_field(name = 'Введіть запитання!(Питання можуть бути будь-які і відповідь буде рандомна!)', value = 'Приклад: ***.ball Питання***')
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)
    else:
        ...
        emb = discord.Embed(title = "Гра кулька", colour = discord.Color.green(), timestamp=ctx.message.created_at)
        emb.add_field(name = f"{random.choice(possible_responses)}", value = "Чекаю на твоїх питань)))")
        emb.set_author(name=ctx.command.description)
        emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
        await ctx.reply(embed = emb)

@bot.command(name = "dice", aliases = ["монетка"])
async def dice(ctx, arg = None):
    if arg == None:
        emb = discord.Embed(title = "Помилка", colour = discord.Color.red(), timestamp=ctx.message.created_at)
        emb.add_field(name = "Потрібно ввести що ви загадали!", value = f"{prefix}dice Орел/Решка(з великої літери!)")
        ...
        if pl1 == pl2:
            if pl2 == "Решка":
                emb = discord.Embed(title = "Зачейкайте, йде вибір!", colour = discord.Color.green(), timestamp=ctx.message.created_at)
                emb.set_image(url = 'https://media.giphy.com/media/p9QEQ7UHzEQ1zoYjtN/giphy.gif')
                emb.set_author(name=ctx.command.description)
                emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
                message = await ctx.reply(embed = emb)
                await asyncio.sleep(5)
                emb = discord.Embed(title = "Ти переміг! Вітаю!", colour = discord.Color.green(), timestamp=ctx.message.created_at)
            ...
            if pl2 == "Решка":
                ...
                message = await ctx.reply(embed = emb)
                await asyncio.sleep(5)
                emb = discord.Embed(title = "Ти програв :(", colour = discord.Color.green(), timestamp=ctx.message.created_at)
                emb.add_field(name = "Ти загадав:", value = f"`{pl1}`")
                emb.add_field(name = "Тобі випало:", value = f"`{pl2}`")
                emb.set_author(name=ctx.command.description)
                emb.set_footer(text="Відправлено " + ctx.author.display_name, icon_url=ctx.author.avatar_url)
...
@bot.command(name = "love", aliases = ["кохання"])
async def love(ctx):
